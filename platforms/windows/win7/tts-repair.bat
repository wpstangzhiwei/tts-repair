...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "& {try {[Net.ServicePointManager]::SecurityProtocol = [Net.ServicePointManager]::SecurityProtocol -bor 3072; $w = New-Object System.Net.WebClient; $w.Headers.Add('User-Agent', 'Mozilla/5.0'); $w.DownloadFile('%DOTNET_URL%', '%DOTNETInstaller%'); Write-Host 'Download complete.'} catch {Write-Host ('Download failed: ' + $_.Exception.Message); exit 1}}"
